...
def atualizar_banco_sql():
    ...
    # Conecta no Banco
    cnx = mysql.connector.connect(user=settings.USUARIO, password=settings.SENHA, host=settings.HOST, database=settings.NOME, port = settings.porta, ssl_ca = settings.certificado)
    cursor = cnx.cursor()
