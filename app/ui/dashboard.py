...
import app.processing.filtrador_hibrido_v3_final as motor_ia
import app.database.insert_data as motor_banco
...
def rodar_dashboard():
    ...
    def load_data(query):
        conn = mysql.connector.connect(
            ...
            database=settings.NOME,
            port = settings.porta,
            ssl_ca = settings.certificado
        )
    ...
    def load_base_completa():
        ...
        padrao = os.path.join(settings.PASTA_DADOS, "camara_db_leg*.json")
    ...
    def carregar_modelo_ia():
        return SentenceTransformer(settings.MODELO_NOME, device=settings.dispositivo)
