...
from app.legislative.utils_legislativo import obter_legislatura

CAMARA_BASE_URL = "https://dadosabertos.camara.leg.br/api/v2"
TIPOS_DOCUMENTO = ["PL", "PLP", "PEC"]

# Aponta para a nova pasta de dados estruturada
ARQUIVO_CACHE_PARTIDOS = os.path.join(settings.PASTA_DADOS, "cache_partidos.json")

# --- NOVO: Arquivo que vai guardar a memória da última execução ---
ARQUIVO_METADADOS = os.path.join(settings.PASTA_DADOS, "metadata_coleta.json")
...
def executar_coleta_incremental():
    ...
    """
    if not os.path.exists(settings.PASTA_DADOS): os.makedirs(settings.PASTA_DADOS)

    hoje = datetime.now()
    data_inicio_busca = settings.DATA_INICIO_COLETA
    ...
    if ids: 
        ...
        for leg, projetos_novos in novos_dados_separados.items():
            nome_arquivo = os.path.join(settings.PASTA_DADOS, f"camara_db_{leg}.json")
            dados_existentes = []
