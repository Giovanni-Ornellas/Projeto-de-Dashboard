...
from app.legislative.utils_legislativo import limpar_ementa_para_vetorizacao, limpar_texto_basico, validar_tag
from app.semantic.embeddings import get_model, get_or_create_embeddings
...
NOME_ARQUIVO_SAIDA = os.path.join(settings.PASTA_CSV, "proposicoes_camara_resumo.csv")
...
def processar_lote(dados, pkl_data, query_embedding, query_embedding_secundaria, termos_usuario, model, sufixo_leg):
    ...
    for idx, score_tensor in enumerate(cos_scores_principal):
        ...
        if score_sem_principal < settings.THRESHOLD_SEMANTICO_MINIMO:
            continue
        if query_embedding_secundaria is not None and score_sem_secundaria < settings.THRESHOLD_SEMANTICO_MINIMO_SECUNDARIA:
            continue

        # MÉDIA PONDERADA
        score_sem_combinado = ((score_sem_principal * settings.PESO_QUERY_PRINCIPAL) + 
                               (score_sem_secundaria * settings.PESO_QUERY_SECUNDARIA))
        ...
        # ----------------------------
        final = (score_sem_combinado * settings.PESO_SEMANTICO) + (score_kw * settings.PESO_KEYWORD)
        
        if final >= settings.FILTRO_THRESHOLD:
            # ----------------------------
...
def executar_filtragem(consulta_usuario, consulta_secundaria, model):
    ...
    padrao_busca = os.path.join(settings.PASTA_DADOS, "camara_db_leg*.json")
    ...
    for arquivo in arquivos_db:
        ...
        arquivo_pkl = os.path.join(settings.PASTA_DADOS, f"keywords_embeddings_{sufixo_leg}.pkl")
...
# 4. Ordenação e Re-ranking
    ...
    candidatos = todos_resultados[:settings.TOP_K_RERANK]
