...
from app.legislative.utils_legislativo import limpar_ementa_para_vetorizacao
...
def get_or_create_embeddings(dados, sufixo_leg, model):
    ...
    """
    arquivo_cache = os.path.join(settings.PASTA_DADOS, f"cache_ementas_{sufixo_leg}.pkl")
    caminho_json = os.path.join(settings.PASTA_DADOS, f"camara_db_{sufixo_leg}.json")
...
def main():
    ...
    if not arquivos_json:
        print(f"Erro: Nenhum arquivo JSON encontrado em {settings.PASTA_DADOS}", flush = True)
        return
